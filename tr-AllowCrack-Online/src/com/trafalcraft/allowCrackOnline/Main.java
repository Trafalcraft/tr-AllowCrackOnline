...
public class Main extends Plugin {
	...
	public void onEnable() {
		...
		getProxy().getPluginManager().registerListener(this, new ACListener(this, "Invalid username. Hacking?"));
		getLogger().info("Plugin initialisé");
		...
		try {
			setConfig(ConfigurationProvider
					.getProvider(YamlConfiguration.class).load(
							new File(getDataFolder(), "config.yml")));
		} catch (IOException e) {
			e.printStackTrace();
		}
		getLogger().info("Config initialisé");
        ...
        try {
        	db.createStatement().executeUpdate("CREATE TABLE IF NOT EXISTS `" + getConfig().getString("database.prefix") + "users` (`name` VARCHAR(10) NOT NULL, `pass` varchar(32), `lastIP` varchar(15), `lastAuth` varchar(16), PRIMARY KEY (`name`), UNIQUE(`name`), INDEX(`name`)) CHARACTER SET utf8");

            ResultSet rs = db.createStatement().executeQuery("SELECT COUNT(*) FROM `" + getConfig().getString("database.prefix") + "users`");
            ResultSet rs2 = db.createStatement().executeQuery("SELECT * FROM `" + getConfig().getString("database.prefix") + "users`");
            
        	while (rs.next()) if (rs.getInt(1) > 0) {
        		for(int i = 0;i<rs.getInt(1);i++){
        			rs2.next();
        			mc.addPlayerCache(rs2.getString(1), rs2.getString(2), rs2.getString(3), rs2.getString(4));
	...
	public void onDisable() {
		for(ProxiedPlayer p : getProxy().getPlayers()){
			p.disconnect(TextComponent.fromLegacyText("Vous avez été déconnectez \n redémarrage du proxy"));
		}
	}
